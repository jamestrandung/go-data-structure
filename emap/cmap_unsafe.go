...
// ConcurrentMapUnsafeKey is almost the same as ConcurrentMap but different in that it
// can accept unsafe interface{} keys. Clients may run into panics if they write into
// this map using non-comparable keys.
...
func NewConcurrentMapUnsafeKey[V any]() ConcurrentMapUnsafeKey[V] {
    return NewConcurrentMapUnsafeKeyWithConcurrencyLevel[V](defaultShardCount)
}
...
func (cm ConcurrentMapUnsafeKey[V]) takeSnapshot() []<-chan ds.Tuple[interface{}, V] {
    ...
    if len(cm) == 0 {
        panic(`emap.ConcurrentMapUnsafeKey is not initialized. Should run NewConcurrentMapUnsafeKey() before usage.`)
    }
